// Service Worker Registration for Apple House Store
  ...
  if ('serviceWorker' in navigator && import.meta.env.PROD) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/serviceWorker.js')
        .then(registration => {
          console.log('Apple House Store SW registered successfully:', registration.scope);
        ...
        .catch(error => {
          console.log('Apple House Store SW registration failed:', error);
        });
    });
...
export function checkInstallPrompt() {
  ...
  window.addEventListener('appinstalled', (evt) => {
    console.log('Apple House Store was installed');
  });
}
